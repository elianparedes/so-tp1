...
int main(int argc, char const *argv[])
{
    ...
    while (files_in_shm < (argc - 1))
    {
        for (int i = 0; i < SLAVES; i++)
        {
            FD_SET(slave_to_master[i][READ_END], &read_set_fds);
            FD_SET(master_to_slave[i][WRITE_END], &write_set_fds);
        }
        ...
        for (int i = 0; i < SLAVES; i++)
        {
            if (FD_ISSET(slave_to_master[i][READ_END], &read_set_fds))
            {
                ...
                if (files_in_hash < argc){
                    ...
                    strcat(input_buffer, argv[files_in_hash++] );
                    strcat(input_buffer, " \n");
                    if (write(master_to_slave[i][WRITE_END], input_buffer, strlen(input_buffer) +1) == ERROR_CODE){
                        perror("app | write");
                    }
